...
function main()
	...
	sampRegisterChatCommand("rv", function (id)
		...
		if not isCharInAnyCar(PLAYER_PED) then
			return sampAddChatMessage("LUA: Вы должны находиться в транспорте.", 0x007FFF)
		end
	...
	sampRegisterChatCommand("rvset", function (params)
	...
	function events.onSendPlayerSync()
		if rv then
			reset()
			sampAddChatMessage("LUA: Вы покинули транспорт, рванка автоматически выключена.", 0x007FFF)
		end
	end
